...
public abstract class ColorProcessor implements VisionProcessor {
    ...
    // For drawing over top of the Android canvas
    //public Mat overlay;
    ...
    public Object processFrame(Mat frame, long captureTimeNanos) {
        ...
        {
            double a = Imgproc.contourArea(contours.get(j));
            if(biggestArea < a  && a > minArea()){
                biggestArea = a ;
                biggestIndex = j;}
        }
    ...
    public void onDrawFrame(Canvas canvas, int onscreenWidth, int onscreenHeight, float scaleBmpPxToCanvasPx, float scaleCanvasDensity, Object userContext) {
        ...
        Imgproc.drawContours(overlay, contours, -1, new Scalar(255,255,0),5);

        camOut = Bitmap.createBitmap(w, h, Bitmap.Config.RGB_565);
        Utils.matToBitmap(overlay, camOut);
        ...
        /*p.setColor(Color.YELLOW);
        for (MatOfPoint contour : (List<MatOfPoint>) userContext) {
            Point[] pts = contour.toArray();


            Arrays.stream(pts).map(pt -> {return new float[(float) pt.x * scaleBmpPxToCanvasPx, (float) pt.y * scaleBmpPxToCanvasPx];});
            canvas.drawPoints();
        }*/
    }
